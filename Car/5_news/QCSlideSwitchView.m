...
- (void)initValues
{
    ...
    _topScrollView.backgroundColor = RGBColor(243, 243, 243);
...
- (void)createNameButtons
{
    ...
    for (int i = 0; i < [_viewArray count]; i++) {
        ...
        if ([_viewArray count] <= 2 ){
            textSize.width = (320.0 - kWidthOfButtonMargin*([_viewArray count]+1))/[_viewArray count];
        }
        
        
        
        LSLog(@" %d :%f %f",i+1,textSize.height,textSize.width);
...
//        button.layer.borderColor =[[UIColor blueColor] CGColor];
        if (i != ([_viewArray count] -1)){
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(xOffset - 8.0,(kHeightOfTopScrollView -20)/2 , 1 , 20)];
